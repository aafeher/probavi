...
func TestResolveXtraBackupSource(t *testing.T) {
	backup := writeBackupFixture(t)
	src, perr := resolveSource("xtrabackup", backup)
	if perr != nil {
	...
	if _, perr := resolveSource("xtrabackup", file); perr == nil || perr.Code != "invalid_request" {
		t.Errorf("file as backup dir: %+v, want invalid_request", perr)
	}
	if _, perr := resolveSource("xtrabackup", filepath.Join(t.TempDir(), "gone")); perr == nil || perr.Code != "source_not_found" {
	...
	if _, perr := resolveSource("xtrabackup", notBackup); perr == nil || perr.Code != "source_corrupt" ||
