// Command probavi-adapter-mssql is the Microsoft SQL Server engine adapter for
// Probavi, implementing probavi-adapter/0 (docs/adapter-protocol.md).
//
// Like the other in-repo adapters it is deliberately self-contained:
// standard library only, no imports from the Probavi core — an adapter
// must be writable from the protocol document alone.
package main
