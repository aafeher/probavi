...
func TestExecRunnerReal(t *testing.T) {
	// The real runner needs no Docker — any binary proves the contract.
	r := execRunner{}

	stdout, _, truncated, exit, err := r.run(context.Background(), nil, "sh", "-c", "echo hello")
	...
	_, _, _, exit, err = r.run(context.Background(), nil, "sh", "-c", "exit 3")
	if err != nil || exit != 3 {
		t.Errorf("exit 3: exit=%d err=%v (non-zero exit is a result, not an error)", exit, err)
	}

	stdout, _, _, _, err = r.run(context.Background(), strings.NewReader("piped"), "cat")
	...
	stdout, _, truncated, _, err = r.run(context.Background(), nil, "sh", "-c", big)
	...
	cancel()
	if _, _, _, _, err := r.run(ctx, nil, "sleep", "1"); err == nil {
		t.Error("canceled context must be an error")
	}

	if _, _, _, _, err := r.run(context.Background(), nil, "/no/such/binary-probavi"); err == nil {
