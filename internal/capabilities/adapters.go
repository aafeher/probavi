...
// VerifiedImage returns the image the integration suite restores from.
func (m *AdapterManifest) VerifiedImage() (string, error) {
	if len(m.Verified) == 0 || m.Verified[0].Image == "" {
		return "", fmt.Errorf("adapter %s: manifest declares no verified image", m.ID)
	}
	return m.Verified[0].Image, nil
}
