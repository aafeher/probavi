...
- **A sigstore build-provenance attestation** on every archive and
  ...
  ```console
  $ gh attestation verify probavi_0.3.1_amd64.deb --repo probavi/probavi
  ```
...
```console
$ sudo dnf install ./probavi-0.3.1-1.x86_64.rpm ./probavi-adapter-postgres-0.3.1-1.x86_64.rpm
```
...
```console
$ sudo apk add --allow-untrusted ./probavi_0.3.1_x86_64.apk ./probavi-adapter-postgres_0.3.1_x86_64.apk
```
...
```console
$ curl -fsSLO "https://github.com/probavi/probavi/releases/download/v0.3.1/PKGBUILD"
$ makepkg -si
```
...
$ cp probavi-0.3.1.ebuild /var/db/repos/local/app-backup/probavi/
$ ebuild /var/db/repos/local/app-backup/probavi/probavi-0.3.1.ebuild manifest
...
```console
$ ver=0.3.1 arch=arm64        # or amd64 on an Intel Mac
$ base="https://github.com/probavi/probavi/releases/download/v${ver}"
...
$ curl -fsSL -o "$(brew --repository "$USER/probavi")/Formula/probavi.rb" \
    "https://github.com/probavi/probavi/releases/download/v0.3.1/probavi.rb"
$ curl -fsSL -o "$(brew --repository "$USER/probavi")/Formula/probavi-adapter-postgres.rb" \
    "https://github.com/probavi/probavi/releases/download/v0.3.1/probavi-adapter-postgres.rb"
